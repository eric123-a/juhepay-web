...
export const boundRoad = (appId:string,platformChannelCodes:any) =>
    request({
        url:`/merchant/my/apps/${appId}/platform-channels?tenantId=${UserModule.tenantId}&platformChannelCodes=alipay_wap`,
        method:'post'
    })
...
export const getStoresDataCode =  (appId:string,storeId:string,body:string,totalAmount:string) => 
    request ({
        url:`/merchant/my/apps/${appId}/stores/${storeId}/app-store-qrcode?tenantId=${UserModule.tenantId}`,
        method:'get'
    })
//查询应用是否绑定了某个服务类型
